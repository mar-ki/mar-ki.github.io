...
During enumeration I encountered an open UDP Port on Port 161 (SNMP)
Let's check if it will reveal any details about the environment
...
> More infos on https://book.hacktricks.xyz/network-services-pentesting/pentesting-web/403-and-401-bypasses#http-verbs-methods-fuzzing
...
Visiting http://10.13.37.11/5000/console will display the debugging console which is protected by a pin. Since we already found a LFI and are able to identify which python version is used I will use that to generate a pin based on system infos (https://book.hacktricks.xyz/network-services-pentesting/pentesting-web/werkzeug)

First I get infos that are needed to generate the pin

MAC: http://10.13.37.11:5000/file?filename=/sys/class/net/ens33/address (Convert using: http://vaultr.com)
...
Sudo V. 1.8.21p2 can be exploited using a known exploit **CVE-2019-18634** - https://github.com/saleemrashid/sudo-cve-2019-18634
...
I used https://gchq.github.io/CyberChef/ to identify that it's an base64 encoded string and used Cyberchef to decode it
...
I was able to identify the most probable ciphers using https://www.dcode.fr/cipher-identifier 
...
Using https://www.dcode.fr/vigenere-cipher was pretty easy as I just had to modify the alphabet of used letters and the **Automatic Decryption** did the rest
